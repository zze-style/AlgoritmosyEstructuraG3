...
public class Arbol<T extends Comparable<T>> {
    ...
    protected Nodo<T> insertarRec(Nodo<T> actual, T dato) {
        if (actual == null) {
    ...
    protected Nodo<T> removeRec(Nodo<T> actual, T dato) {
        if (actual == null) {
            return null;
        }
    ...
    protected Nodo<T> obtenerMenor(Nodo<T> actual) {
    ...
    public int altura() {
        ...
        QueueArray<Nodo<T>> cola = new QueueArray<>(1000);
        cola.enqueue(raiz);
        int altura = -1;

        while (!cola.isEmpty()) {
            int nivel = colaSize(cola);
            altura++;
    ...
    private int colaSize(QueueArray<Nodo<T>> cola) {
        QueueArray<Nodo<T>> aux = new QueueArray<>(1000);
        int count = 0;
    ...
    private boolean isValidRec(Nodo<T> nodo, T min, T max) {
        ...
        return isValidRec(nodo.izquierdo, min, nodo.dato)
                && isValidRec(nodo.derecho, nodo.dato, max);
    }
    ...
    public ListLinked<T> inOrdenLista() {
        ListLinked<T> datos = new ListLinked<>();
        inOrdenLista(raiz, datos);
        return datos;
    }

    private void inOrdenLista(Nodo<T> nodo, ListLinked<T> datos) {
        if (nodo != null) {
            inOrdenLista(nodo.izquierdo, datos);
            datos.insertLast(nodo.dato);
            inOrdenLista(nodo.derecho, datos);
        }
    }

    public ListLinked<T> preOrdenLista() {
        ListLinked<T> datos = new ListLinked<>();
        preOrdenLista(raiz, datos);
        return datos;
    }

    private void preOrdenLista(Nodo<T> nodo, ListLinked<T> datos) {
        if (nodo != null) {
            datos.insertLast(nodo.dato);
            preOrdenLista(nodo.izquierdo, datos);
            preOrdenLista(nodo.derecho, datos);
        }
    }

    public ListLinked<T> postOrdenLista() {
        ListLinked<T> datos = new ListLinked<>();
        postOrdenLista(raiz, datos);
        return datos;
    }

    private void postOrdenLista(Nodo<T> nodo, ListLinked<T> datos) {
        if (nodo != null) {
            postOrdenLista(nodo.izquierdo, datos);
            postOrdenLista(nodo.derecho, datos);
            datos.insertLast(nodo.dato);
        }
    }
